#!/usr/bin/env python3
# -*- coding: utf-8 -*-
import re, urllib.request, urllib.parse, time, sys, json
...
LIMIT = 10  # process only 10 comment lines
...
def main():
    ...
    print(f"Processing first {len(matches)} matches")
    # Group by file
    ...
    for file_path, items in by_file.items():
        ...
        if not full_path.exists():
            ...
            continue
        print(f"Processing {file_path} with {len(items)} lines")
        ...
        for line_num, snippet in items:
            ...
            if not chinese_parts:
                print(f"No Chinese in line {line_num}")
                continue
            replacements = {}
            for ch in chinese_parts:
                if ch not in replacements:
                    print(f"Translating '{ch}'...")
