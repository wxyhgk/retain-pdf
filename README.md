...
外文论文、教材、技术文档虽然信息很多，但真正读起来往往很累：

- 直接看原文，阅读门槛高，效率低
- 普通翻译工具往往只给纯文本，公式、图片、排版结构很容易全乱
- 想继续整理、分享、归档，也不方便

这个项目解决的就是这些问题：

- 上传 PDF 后，可以一键生成保留排版的译文
- 尽量保留原始版式，而不是只给一段散乱的纯文字翻译
- 除了译文 PDF，还可以拿到 Markdown 和 ZIP 打包结果
- 既能给普通用户直接用网页操作，也支持命令行和 API 接入

默认镜像地址：

- `wxyhgk/retainpdf-app:latest`
- `wxyhgk/retainpdf-web:latest`
