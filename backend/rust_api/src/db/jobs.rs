...
impl Db {
    ...
    pub fn recover_stale_running_job(
        ...
        let failure = JobFailureInfo {
            ...
            provider_stage: None,
            provider_code: None,
            summary: "Khi khởi động backend đã thu hồi tác vụ running còn sót lại".to_string(),
            root_cause: Some(detail.to_string()),
            retryable: true,
            upstream_host: None,
            provider: None,
            suggestion: Some("Worker tương ứng với tác vụ này không còn chạy; vui lòng gửi lại hoặc thử lại thủ công".to_string()),
