...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Migrate a legacy OCR-provider experiment layout under output/mineru/<case> into the structured output/<job-id>/{originPDF,jsonPDF,transPDF} layout.",
    )
    parser.add_argument(
        "--legacy-root",
        type=str,
        default="output/mineru/test9",
        help="Legacy provider experiment directory to migrate.",
    )
    parser.add_argument(
        "--job-id",
        type=str,
...
def main() -> None:
    ...
    notes = [
        "This job directory was created by migrating the old output/mineru/<case> provider experiment layout.",
        f"legacy root: {legacy_root}",
        "jsonPDF/unpacked contains the original unpacked bundle and raw provider files for adapter/debug use.",
