...
#[tokio::test]
async fn translation_item_route_redacts_secrets() {
    let state = test_state("debug-item-redaction");
    ...
    assert_eq!(payload["data"]["item"]["api_key"], "");
    assert_eq!(payload["data"]["item"]["source_text"], "English [REDACTED]");
}
