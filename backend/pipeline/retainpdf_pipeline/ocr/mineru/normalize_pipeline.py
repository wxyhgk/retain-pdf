...
from retainpdf_pipeline.services.pipeline_shared.contracts import (
    ...
    STDOUT_LABEL_PROVIDER_RAW_DIR,
    STDOUT_LABEL_PROVIDER_SUMMARY_JSON,
    STDOUT_LABEL_PROVIDER_ZIP,
    STDOUT_LABEL_SCHEMA_VERSION,
    STDOUT_LABEL_SOURCE_PDF,
    STDOUT_SCHEMA_VERSION_VALUE,
    format_stdout_kv,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--layout-json", type=str, required=True, help="Path to MinerU layout.json"
    )
    parser.add_argument(
        "--source-pdf", type=str, required=True, help="Path to source PDF"
    )
    parser.add_argument(
        "--job-id", type=str, default="", help="Optional explicit job directory name"
    )
    parser.add_argument(
        "--output-root",
        type=str,
...
def main() -> None:
    ...
    if not layout_json_path.exists():
        provider_raw_dir = str(args.provider_raw_dir or "").strip()
        discovery_root = (
            Path(provider_raw_dir).resolve()
            if provider_raw_dir
            else layout_json_path.parent
        )
        layout_json_path = resolve_layout_json_path(discovery_root).resolve()
    ...
    if args.provider_result_json.strip():
        ...
        if (
            provider_result_json.exists()
            and provider_result_json != artifact_paths.result_json_path.resolve()
        ):
    ...
    if args.provider_zip.strip():
        ...
        if (
            provider_zip_path.exists()
            and provider_zip_path != artifact_paths.bundle_zip_path.resolve()
        ):
    ...
    print(
        format_stdout_kv(
            STDOUT_LABEL_NORMALIZED_DOCUMENT_JSON, artifact_paths.normalized_json_path
        ),
        flush=True,
    )
    print(
        format_stdout_kv(
            STDOUT_LABEL_NORMALIZATION_REPORT_JSON, normalization_report_path
        ),
        flush=True,
    )
    print(
        format_stdout_kv(
            STDOUT_LABEL_PROVIDER_RAW_DIR,
            args.provider_raw_dir.strip() or artifact_paths.unpack_dir,
        ),
        flush=True,
    )
    print(
        format_stdout_kv(
            STDOUT_LABEL_PROVIDER_ZIP,
            args.provider_zip.strip() or artifact_paths.bundle_zip_path,
        ),
        flush=True,
    )
    print(
        format_stdout_kv(
            STDOUT_LABEL_PROVIDER_SUMMARY_JSON,
            args.provider_result_json.strip() or artifact_paths.result_json_path,
        ),
        flush=True,
    )
    print(
        format_stdout_kv(STDOUT_LABEL_SCHEMA_VERSION, STDOUT_SCHEMA_VERSION_VALUE),
        flush=True,
    )
