...
#let b0_md = "$A _ { k c , k c } ^ { \\prime }$大于能量阈值。另一方面，配置${} _ { i a }$的对角元素$A _ { i a , i a } ^ { \\prime }$小于阈值，因此从一开始就被包含在矩阵$\\mathbf { A } ^ { \\prime }$中。"
...
#let b1_md = "如果配置$k c$的耦合总和大于$E _ { t h r } ^ { ( 2 ) }$，该配置将被包含（默认：$k c$包含所有配置$E _ { t h r } ^ { ( 2 ) } = 1 0 ^ { - 4 } E _ { h } ,$）。"
...
#let b3_md = "以同样的简化方式，完整的TD-DFT问题（方程1）可以通过$[ 2 ]$求解。简化矩阵$\\mathbf { B ^ { \\prime } }$的元素如下："
...
#let b4_md = "$a _ { x }$是密度泛函中非局域Fock交换的量（例如B3LYP中为0.2）。该方法已被证明能产生更可靠的跃迁矩，因此比sTDA获得更好的UV/VIS和ECD光谱（更多信息参见参考文献2）。"
...
#let b5_md = "$\\alpha$和$\\beta$参数（见公式4和5）在sTDA和sTD-DFT中是相同的。因此，$\\gamma _ { A B } ^ { J }$和$\\gamma _ { A B } ^ { K }$在两种方法中也是一样的。对于矩阵$\\mathbf { A } ^ { \\prime }$和$\\mathbf { B ^ { \\prime } }$的维度也是如此，即执行与sTDA相同的构型选择。"
...
#let b7_md = "当通过对外加电场$\\big ( { \\frac { \\partial } { \\partial F _ { \\zeta } } } \\big | _ { \\vec { F } = 0 } \\big )$取一阶导数来开启微扰时，线性响应矩阵方程与方程1类似："
...
#let b8_md = "其中密度矩阵一阶微扰的导数定义了频率相关的响应向量$\\begin{array} { r } { \\frac { \\partial D _ { a i } ^ { ( 1 ) } ( \\omega ) } { \\partial { \\cal F } _ { \\zeta } } | _ { \\vec { F } = 0 } = X _ { \\zeta , a i } ( \\omega ) + Y _ { \\zeta , a i } ( \\omega ) } \\end{array}$，且$\\mu _ { \\zeta , a i } = \\langle \\phi _ { a } | { \\hat { \\mu } } _ { \\zeta } | \\phi _ { i } \\rangle$。在sTD-DFT框架中，用于计算极化率和第一超极化率的线性响应向量[15]通过求解方程8获得："
...
#let b10_md = "在sTD-DFT近似下，二次谐波生成的一阶超极化率表达式为"
