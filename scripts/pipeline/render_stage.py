    ...
    try:
        ...
        for page_idx in sorted(render_pages_map):
            if 0 <= page_idx < len(doc):
                ...
                apply_translated_items_to_page(page, render_pages_map[page_idx], fonts.DEFAULT_FONT_PATH)
    ...
    *,
    ...
    if render_mode in {"compact", "direct"}:
        return render_translated_pages_map(
            ...
            strip_links=(render_mode == "direct"),
        )
...
def run_render_stage(
    *,
    ...
    if effective_render_mode in {"compact", "direct"} and translated_pages_map is not None:
        pages_rendered = render_translated_pages_map(
            source_pdf_path=source_pdf_path,
