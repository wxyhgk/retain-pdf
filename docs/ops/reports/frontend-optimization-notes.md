...
> 历史说明：本文记录的是迁移前 `frontend/` 目录的实现，文中的旧文件名只用于
> 理解当时的拆分过程，不代表当前 `frontend/web/` 目录结构。
...
- 核心交互入口：`frontend/src/js/main.js` 约 `1291` 行
- UI 渲染层：`frontend/src/js/ui.js` 约 `624` 行
- 任务数据整形层：`frontend/src/js/job.js` 约 `424` 行
- 样式主文件：`frontend/src/styles/components.css` 约 `1747` 行
- 前端源码总量约 `224K`
...
- 已新增 `frontend/src/js/desktop-host.js`，由它统一识别 `retainPdfDesktop`，并只保留 `__TAURI_INTERNALS__` 兼容 shim。
- `frontend/src/js/config.js` 不再直接探测旧桥名，桌面调用统一从 host 抽象进入。
...
- 最小补齐：
  - ESLint
  - Prettier
  - 1~2 个最基本的纯函数测试，先覆盖 `frontend/src/js/job.js` 的 normalize/summarize 系列
...
- 缩小 `frontend/src/js/job.js` 的“兼容层”职责
...
1. 把 `frontend/src/js/main.js` 拆掉
2. 把裸 `state` 收口
