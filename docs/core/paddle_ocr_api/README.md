...
1. 不改翻译层 `backend/pipeline/retainpdf_pipeline/translate/*`
2. 不改渲染层 `backend/pipeline/retainpdf_pipeline/render/*`
3. 不在 `backend/pipeline/retainpdf_pipeline/runtime/pipeline/*` 里写 Paddle 私有特判
...
- Paddle adapter 入口：
  `backend/pipeline/retainpdf_pipeline/ocr/document_schema/provider_adapters/paddle/adapter.py`
- Paddle page reader：
  `backend/pipeline/retainpdf_pipeline/ocr/document_schema/provider_adapters/paddle/page_reader.py`
- Paddle block reader：
  `backend/pipeline/retainpdf_pipeline/ocr/document_schema/provider_adapters/paddle/block_reader.py`
- 通用契约说明：
