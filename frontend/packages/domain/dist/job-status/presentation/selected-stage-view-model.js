...
function selectedProgressUnit(progress = {}) {
    return `${progress?.progressUnit || progress?.progress_unit || ""}`.trim();
}
function hasFiniteProgress(progress = {}) {
...
function isConcreteProgressUnit(unit = "") {
    return ["page", "batch", "step", "none"].includes(`${unit || ""}`.trim());
}
function shouldPreferSubstageProgress(progress = {}, substageFallback = null) {
...
export function resolveSelectedStageContext({ snapshot, selectedStageKey = "", }) {
