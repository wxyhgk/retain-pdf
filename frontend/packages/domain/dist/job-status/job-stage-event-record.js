import { canonicalStageOf, } from "./presentation/job-stage-presentation-utils.js";
import { eventIdentity, eventLaneOf, hasStructuredProgress, hasStructuredPublicStage, hasCanonicalEventContract, normalizeUserStage, progressUnitOf, structuredPublicStageOf, } from "./contract/job-stage-event-contract.js";
import { progressFromEvent, progressPercentFromEvent, } from "./progress/job-stage-event-progress.js";
...
function canonicalDisplayStageName(stageKey = "") {
    return stageKey === "translate" ? "translation" : stageKey;
}
function progressTextStageKeyForRecord(record = {}) {
...
export function normalizedStageEventRecord(item = {}) {
    ...
    const displayStage = hasCanonicalContract
        ? firstNonEmptyText(item.display_stage, payload.display_stage, canonicalDisplayStageName(record.publicStage))
        : firstNonEmptyText(item.display_stage, payload.display_stage, item.user_stage, payload.user_stage, canonicalDisplayStageName(record.publicStage), canonicalDisplayStageName(record.userStage));
    ...
    const stageText = record.hasCanonicalEventContract
        ? firstNonEmptyText(progressText, displayStage) || "-"
        : firstNonEmptyText(record.stageDetail, progressText, displayStage, record.rawStage) || "-";
    return {
        ...record,
        displayStage,
        progressText,
...
export function stagePayloadFromEventRecord(job = {}, record = {}) {
