...
import { publicProgressOf, progressWithPercent, } from "./progress/job-stage-progress-adapter.js";
import { jobProgressRecord, } from "./progress/job-stage-job-progress.js";
import { collectLatestCurrentStageProgress, } from "./progress/job-stage-progress-records.js";
import { substageMatchesStage, visualStageKeyForPresentation, } from "./presentation/job-stage-presentation-helpers.js";
import { normalizeDisplayStage, } from "./presentation/job-stage-presentation-utils.js";
...
function clampPublicStageForRunningJob(stageKey, status) {
    if (stageKey === "done" && !isCompletedJobStatus(status)) {
        return "render";
    }
    return stageKey;
}
function snapshotObject(job = {}) {
    return job?.stage_snapshot && typeof job.stage_snapshot === "object"
        ? job.stage_snapshot
        : null;
}
function trustedSnapshot(job = {}) {
...
export function publicStageKeyFromJob(job = {}) {
...
function normalizeSubstageForStage(stageKey = "", value = "") {
...
function publicSubstageKeyFromJob(job = {}, stageKey = "") {
...
function stagePayloadForPresentation(job = {}, stageKey = "", substageKey = "", progress = {}) {
    const progressAny = progress;
    return {
...
function sanitizedJobForStage(job = {}, stageKey = "", substageKey = "") {
    const snapshot = trustedSnapshot(job);
...
function chooseCurrentStageProgress(job = {}, eventsPayload = {}, stageKey = "", substageKey = "") {
...
function hasUsefulProgress(progress = null) {
    if (!progress) {
        return false;
    }
...
function genericProgressRecord(job = {}, stageKey = "", substageKey = "") {
...
function progressForPresentation(job = {}, eventsPayload = {}, stageKey = "", substageKey = "") {
    if (stageKey === "done") {
...
function displayPercentFromProgress(progress = {}) {
    const progressAny = progress;
    const explicit = progressAny.displayPercent ?? progressAny.progressPercent ?? null;
...
export function resolvePublicStagePresentation(job = {}, eventsPayload = {}) {
