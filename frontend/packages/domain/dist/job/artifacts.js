...
export function resolveOriginalPdfBaseName(state = {}) {
    ...
    const bookSummary = (rawResponse.book_summary || {});
    const sourceArtifact = findReadyManifestArtifact(defaultArtifactRuntimePort.cachedManifestFor(state, jobId), "source_pdf");
    const candidates = [
...
export function createArtifactUrlResolver({ resolveApiBase = defaultArtifactUrlConfigPort.resolveApiBase, } = {}) {
...
export function resolveManifestArtifactUrl(manifestPayload, artifactKey, { includeJobDir = false } = {}) {
...
export function resolveJobMarkdownContract(job) {
    ...
    const ready = Boolean(markdown.ready
    ...
    return {
        ...
        imagesBaseUrl: ensureTrailingSlash(resolveResourceUrl(markdown.images_base_url || markdown.images_base_path || artifacts.markdown_images_base_url)),
