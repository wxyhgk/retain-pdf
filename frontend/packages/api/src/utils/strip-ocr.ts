 ...
 */
export function stripOcrSuffix(jobId: string): string {
  if (typeof jobId !== "string") return "";
  const trimmed = jobId.trim();
  if (!trimmed) return "";
  // Equivalent to trimmed.replace(/-ocr$/, "") but makes the edge case explicit.
  return trimmed.endsWith("-ocr") ? trimmed.slice(0, -4) : trimmed;
}
